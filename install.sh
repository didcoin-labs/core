...
alias ark="$HOME/core-didcoin/packages/core/bin/run"
echo 'alias didcoin="$HOME/core-didcoin/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-didcoin"
git clone "https://github.com/didcoin-labs/core" "$HOME/core-didcoin" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/didcoin-labs/core'"

    exit 1
fi

cd "$HOME/core-didcoin"
...
rm -rf "$HOME/.config/@didcoin"
rm -rf "$HOME/.config/@didcoin"
rm -rf "$HOME/.config/didcoin-core"
